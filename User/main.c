...
static const uint16_t noteFreq[] = {262, 294, 330, 349, 392, 440, 494};   /* Do ~ Si，与钢琴模式 7 个按键一一对应 */
...
static void RunAutoPlay(void)
{
    ...
    /* 音符时长内分片等待：退出组合键最迟 20 ms 响应，同时保证看门狗被喂 */
    uint16_t remain_ms = (uint16_t)(d * 100U);
    while (remain_ms != 0U) {
        if (KEY_IsExitPressed()) { BUZZER_Off(); LED_AllOff(); HAL_GPIO_WritePin(GPIOC, GPIO_PIN_13, GPIO_PIN_RESET); delay_ms(200); autoSub = AUTO_SELECT; currentMode = MODE_MENU; return; }
        uint16_t step_ms = (remain_ms > 20U) ? 20U : remain_ms;
        delay_ms(step_ms);
        remain_ms -= step_ms;
    }
    BUZZER_Off();
...
int main(void)
{
    ...
    delay_init(72);
    sys_iwdg_init();    /* 启动独立看门狗（约 2 s），异常卡死时自动复位 */
